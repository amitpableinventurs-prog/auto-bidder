...
export function createApiRouter() {
  ...
  post('/auth/register', async (req: Request, res: Response) => {
    ...
    if (useMemoryStore) {
      const user = devStore.upsertPhoneUser(body.phone, body.name, body.referralCode);
    ...
    const user = await prisma.user.upsert({
      ...
      create: {
        ...
        referredByDistributorCode: body.referralCode,
      },
      update: {
        name: body.name,
        email: body.email,
  ...
  post('/auth/otp/verify', async (req: Request, res: Response) => {
    ...
    if (useMemoryStore) {
      const user = devStore.upsertPhoneUser(body.phone, body.name ?? 'Mobile User', body.referralCode);
      const token = jwt.sign(
        { userId: user.id, role },
        env.JWT_SECRET,
        { expiresIn: '7d' }
  ...
  get('/seller/:userId/leads', async (req: Request, res: Response) => {
    ...
    const leads = listings.flatMap((l: (typeof listings)[number]) => (l.bids || []).map((b: NonNullable<(typeof listings)[number]['bids']>[number]) => ({
        id: b.id,
        name: b.user.name,
        phone: b.user.phone,
  ...
  get('/cities', async (_req: Request, res: Response) => {
    ...
    const allCities = Array.from(new Set([...popular, ...cities.map((c: (typeof cities)[number]) => c.city as string)]))
      .filter(Boolean)
      .sort();
  ...
  router.post('/listings', authenticate, authorize(['BUYER', 'SELLER', 'ADMIN']), wrap(async (req, res) => {
    const body = z
      .object({
        ...
        referralCode: z.string().optional(),
        referredByDistributorCode: z.string().optional(),
    ...
    const distributorCode = body.referredByDistributorCode ?? body.referralCode;

    if (useMemoryStore) {
      const distributor = distributorCode ? devStore.getDistributorByCode(distributorCode) : null;
      const listing = devStore.createListing({
        ...body,
        title,
        referredByDistributorCode: distributor?.distributorCode ?? distributorCode ?? null,
        referredByUserId: distributor?.id ?? body.referredByUserId ?? null,
    ...
    const distributor = distributorCode
      ? await prisma.user.findFirst({ where: { distributorCode, isDistributor: true } })
      : null;
    ...
    const listing = await prisma.listing.create({
      data: {
        ...body,
        title,
        referredByDistributorCode: distributor?.distributorCode ?? distributorCode ?? undefined,
        referredByUserId: distributor?.id ?? body.referredByUserId ?? undefined,
        referralCapturedAt: distributor ? new Date() : undefined,
        referralAttributionExpiresAt: distributor ? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) : undefined,
        distributorCommissionAmount: distributor ? 0 : undefined,
        distributorCommissionStatus: distributor ? 'PENDING' : undefined,
        sellerId: body.sellerId || (req as any).user?.id || 'admin',
        startingBid: body.startingBid ?? Math.max(Math.floor((body.demandPrice ?? 0) * 0.9), 0),
      },
      include: listingInclude(),
  ...
  get('/favorites', async (req: Request, res: Response) => {
    ...
    res.json({ favorites: favorites.map((f: (typeof favorites)[number]) => f.listing) });
  });
  ...
  get('/seller/:userId/stats', async (req: Request, res: Response) => {
    ...
    const listingIds = listings.map((l: (typeof listings)[number]) => l.id);
  ...
  post('/distributor/:userId/activate', authenticate, async (req: Request, res: Response) => {
    const userId = req.params.userId;
    const authUser = (req as any).user;
    if (authUser.userId !== userId && authUser.role !== 'ADMIN') {
      return res.status(403).json({ error: 'Forbidden' });
    ...
    if (useMemoryStore) {
      const user = devStore.activateDistributor(userId);
      return res.json({ user, referralLink: `autobidder.in/list?ref=${user.distributorCode}` });
    }
    const user = await prisma.user.update({
      where: { id: userId },
      data: {
        ...
        distributorCode: `AB-DIST-${userId.slice(-6).toUpperCase()}`,
      },
      select: userSelect(),
    });
    res.json({ user, referralLink: `autobidder.in/list?ref=${user.distributorCode}` });
  });

  get('/distributor/:userId/dashboard', authenticate, async (req: Request, res: Response) => {
    const userId = req.params.userId;
    const authUser = (req as any).user;
    if (authUser.userId !== userId && authUser.role !== 'ADMIN') {
      return res.status(403).json({ error: 'Forbidden' });
    ...
    if (useMemoryStore) {
      return res.json(devStore.getDistributorDashboard(userId));
    }
    const distributor = await prisma.user.findUnique({ where: { id: userId }, select: userSelect() });
    if (!distributor?.isDistributor) return res.status(404).json({ error: 'Distributor not found' });
    const listings = await prisma.listing.findMany({
      where: { referredByUserId: userId },
      include: { seller: { select: { name: true, phone: true } } },
      orderBy: { createdAt: 'desc' },
    });
    const referrals = listings.map((listing: (typeof listings)[number]) => ({
    ...
    res.json({
      ...
      stats: {
        ...
        activeListings: referrals.filter((r: (typeof referrals)[number]) => r.status === 'ACTIVE').length,
        soldListings: referrals.filter((r: (typeof referrals)[number]) => r.status === 'SOLD').length,
        totalEarnings: referrals.reduce((sum: number, r: (typeof referrals)[number]) => sum + r.commissionAmount, 0),
        pendingEarnings: referrals.filter((r: (typeof referrals)[number]) => r.commissionStatus === 'PENDING').reduce((sum: number, r: (typeof referrals)[number]) => sum + r.commissionAmount, 0),
        paidEarnings: referrals.filter((r: (typeof referrals)[number]) => r.commissionStatus === 'PAID').reduce((sum: number, r: (typeof referrals)[number]) => sum + r.commissionAmount, 0),
      },
  ...
  get('/users/:userId/wallet', authenticate, async (req: Request, res: Response) => {
    ...
    if (useMemoryStore) {
        const dashboard = devStore.getUser(userId);
        return res.json({
            balance: dashboard?.referralEarnings ?? 25000,
            transactions: [
              { id: 'tx1', type: 'CREDIT', amount: dashboard?.referralEarnings ?? 5000, title: 'Distributor referral earnings', date: new Date(), status: 'SUCCEEDED' },
            ]
        });
    }

    const [user, commissions, payouts] = await Promise.all([
        prisma.user.findUnique({ where: { id: userId } }),
    ...
    const transactions = [
        ...commissions.map(c => ({ id: c.id, type: 'CREDIT', amount: c.amount, title: `Commission: ${c.details || 'Referral'}`, date: c.createdAt, status: 'SUCCEEDED' })),
        ...payouts.map(p => ({ id: p.id, type: 'DEBIT', amount: p.amount, title: 'Withdrawal Request', date: p.createdAt, status: p.status }))
  ...
  post('/users/:userId/payout', authenticate, async (req: Request, res: Response) => {
    const userId = req.params.userId;
    const body = z.object({
        amount: z.number().int().positive(),
    ...
    if (!user || (user.referralEarnings ?? 0) < body.amount) {
    ...
    await prisma.user.update({
        where: { id: userId },
        data: { referralEarnings: { decrement: body.amount } }
    });
  ...
  post('/admin/login', async (req, res) => {
    ...
    const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'admin@autobidder.com';
  ...
  // Admin: Update user (admin-prefixed alias)
  patch('/admin/users/:id', async (req, res) => {
    const id = req.params.id;
    const body = z.object({
      name: z.string().optional(),
      email: z.string().email().optional(),
    ...
    const user = await prisma.user.update({ where: { id }, data: body, select: userSelect() });
    res.json({ user });
  });

  // Admin: Dealers (using verified sellers as dealers)
  get('/admin/dealers', async (_req, res) => {
    if (useMemoryStore) {
      const all = devStore.adminAllUsers();
      const dealers = (all.users || []).filter((u: any) => u.userType === 'DEALER' || u.role === 'SELLER');
      return res.json({ dealers });
    }
    const dealers = await prisma.user.findMany({
      where: { OR: [{ userType: 'DEALER' }, { role: 'SELLER' }] },
      orderBy: { createdAt: 'desc' },
  ...
  patch('/admin/dealers/:id/status', async (req, res) => {
    const id = req.params.id;
    const { status } = z.object({ status: z.string() }).parse(req.body);
    if (useMemoryStore) {
      return res.json({ ok: true });
    }
    const user = await prisma.user.update({ where: { id }, data: { kycStatus: status }, select: userSelect() });
    res.json({ dealer: user });
  });

  patch('/admin/dealers/:id/promote', async (req, res) => {
    const id = req.params.id;
    z.object({ isFeatured: z.boolean() }).parse(req.body);
    if (useMemoryStore) {
      return res.json({ ok: true });
    }
    res.json({ ok: true });
  ...
  // Admin: Leads
  get('/admin/leads', async (_req, res) => {
    if (useMemoryStore) {
    ...
    const leads = listings.flatMap(l => (l.bids || []).map(b => ({
      id: b.id, name: b.user.name, phone: b.user.phone,
      listingTitle: l.title, timeAgo: 'Recently',
    })));
    res.json({ leads });
  });

  // Admin: Commissions
  get('/admin/commissions', async (_req, res) => {
    if (useMemoryStore) {
      return res.json({ commissions: [], stats: { total: 0, collected: 0, pending: 0 } });
    }
    const payments = await prisma.payment.findMany({
      where: { status: 'SUCCEEDED' },
    ...
    const commissions = payments.map(p => ({
      id: p.id,
      listingTitle: p.listing?.title || 'Unknown',
      amount: Math.floor(p.amount * 0.02),
      totalSale: p.amount,
      status: 'PAID',
      date: p.createdAt,
    }));
    res.json({ commissions, stats: { total: commissions.length, collected: commissions.length, pending: 0 } });
  });

  // Admin: Payouts
  get('/admin/payouts', async (_req, res) => {
    if (useMemoryStore) {
      return res.json({ payouts: [], stats: { pending: 0, completed: 0, total: 0 } });
    }
    const payments = await prisma.payment.findMany({
      where: { status: 'SUCCEEDED' },
      include: { listing: { include: { seller: { select: { name: true, phone: true } } } } },
      orderBy: { createdAt: 'desc' },
    });
    const payouts = payments.map(p => ({
      id: p.id,
      sellerName: p.listing?.seller?.name || 'Unknown',
      sellerPhone: p.listing?.seller?.phone || '',
      listingTitle: p.listing?.title || 'Unknown',
      amount: Math.floor(p.amount * 0.98),
      status: 'COMPLETED',
      date: p.createdAt,
    }));
    res.json({ payouts, stats: { pending: 0, completed: payouts.length, total: payouts.length } });
  });

  // Admin: Verification pending (sellers)
  get('/admin/users/verification-pending', async (_req, res) => {
    if (useMemoryStore) {
      const all = devStore.adminAllUsers();
      const users = (all.users || []).filter((u: any) => !u.isVerified);
  ...
  // Admin: Verification pending (listings)
  get('/admin/listings/verification-pending', async (_req, res) => {
    if (useMemoryStore) {
      const listings = devStore.listListings({ status: 'PENDING_INSPECTION' });
  ...
  post('/admin/notifications/send', async (req: Request, res: Response) => {
    ...
    if (userId) {
      ...
      await prisma.notification.createMany({
        data: users.map((u: (typeof users)[number]) => ({ userId: u.id, title, message, type: 'SYSTEM' }))
      });
    }
  ...
  get('/admin/analytics', async (_req: Request, res: Response) => {
    ...
    const revenue = payments.reduce((acc: number, p: (typeof payments)[number]) => acc + p.amount, 0);
  ...
  // Admin: Distributors
  get('/admin/distributors', async (req: Request, res: Response) => {
    if (useMemoryStore) {
      return res.json({ distributors: devStore.getDistributors() });
    }
    ...
    const withCounts = await Promise.all(distributors.map(async (user: (typeof distributors)[number]) => ({
      ...user,
      referralCount: await prisma.listing.count({ where: { referredByUserId: user.id } }),
    })));
    res.json({ distributors: withCounts });
  });

  patch('/admin/distributors/:id/status', async (req: Request, res: Response) => {
    const id = req.params.id;
    const { status } = z.object({ status: z.enum(['ACTIVE', 'SUSPENDED']) }).parse(req.body);
    if (useMemoryStore) {
      return res.json({ distributor: devStore.updateDistributorStatus(id, status) });
    }
    const distributor = await prisma.user.update({
      where: { id },
      data: { isDistributor: true, distributorStatus: status },
      select: userSelect(),
    });
  ...
  get('/admin/payouts', async (req: Request, res: Response) => {
    ...
    const formatted = payouts.map(p => ({
        ...
        role: p.user.role,
        accountNumber: (p.bankDetails as any)?.accountNumber || '—',
        ifsc: (p.bankDetails as any)?.ifsc || '—',
    }));
  ...
  get('/admin/commissions', async (req: Request, res: Response) => {
    ...
    const formatted = commissions.map(c => ({
      ...c,
      dealerName: c.user?.name || 'Platform',
      listingTitle: c.listing?.title || '—',
      rate: c.rate || 0,
      earned: c.amount,
    }));
    res.json({ commissions: formatted, totalEarned: formatted.reduce((s, c) => s + c.earned, 0) });
  });
...
patch('/admin/listings/:id/status', async (req: Request, res: Response) => {
    ...
    let distributorCommissionAmount = existing?.distributorCommissionAmount ?? 0;
    let distributorCommissionStatus = existing?.distributorCommissionStatus ?? 'NONE';
    if (existing?.referredByUserId) {
      let amount = 0;
      let commissionStatus = 'PENDING';
      if (status === 'ACTIVE') amount += 1000;
      if (status === 'SOLD') amount += Math.floor(((existing.demandPrice ?? existing.startingBid) || 0) * 0.02);
      if (status === 'REJECTED') commissionStatus = 'REJECTED';
      distributorCommissionAmount = amount;
      distributorCommissionStatus = commissionStatus;

      if (amount > (existing.distributorCommissionAmount ?? 0)) {
        const delta = amount - (existing.distributorCommissionAmount ?? 0);
        await prisma.commission.create({
          data: {
            userId: existing.referredByUserId,
            listingId: existing.id,
            amount: delta,
            type: 'DISTRIBUTOR',
            status: commissionStatus === 'REJECTED' ? 'REJECTED' : 'ACCRUED',
            details: `Commission for listing status: ${status}`,
          },
        });

        await prisma.user.update({
          where: { id: existing.referredByUserId },
          data: {
            referralEarnings: {
              increment: delta,
            },
    ...
    const listing = await prisma.listing.update({
      where: { id },
      data: { status, distributorCommissionAmount, distributorCommissionStatus },
      include: listingInclude()
    });
    res.json({ listing });
  });
