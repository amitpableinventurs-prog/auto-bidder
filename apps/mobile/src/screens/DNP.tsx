...
import { activateDistributor, getDistributorDashboard, getSellerStats, getLeads, updateUser } from '../api';
...
export default function DNPScreen() {
  ...
  const fetchData = async () => {
    ...
    if (user?.id) {
      try {
        const [statsRes, leadsRes, distributorRes] = await Promise.all([
          getSellerStats(user.id).catch(() => MOCK_STATS),
          getLeads(user.id).catch(() => ({ leads: MOCK_LEADS })),
          (user?.isDistributor ? getDistributorDashboard(user.id) : Promise.resolve(null)).catch(() => null)
        ]);
        setStats(statsRes || MOCK_STATS);
        setLeads(leadsRes.leads || MOCK_LEADS);
        setDistributorData(distributorRes);
      } catch (err) {
  ...
  const referralRows = useMemo(() => distributorData?.referrals || [], [distributorData]);

  const handleBecomeDistributor = async () => {
    if (!user?.id) return;
    try {
      setLoading(true);
      await activateDistributor(user.id);
      Alert.alert('Distributor activated', 'Your referral link is ready. Start sharing and earning.');
      await fetchData();
    } catch (err: any) {
  ...
  const handleShareDistributorLink = async () => {
    const message = distributorData?.shareTemplates?.appInvite || `Join AutoBidder with my referral link: ${distributorData?.referralLink || ''}`;
    try {
  ...
  const renderDistributorCard = () => {
    if (!user?.isDistributor && !distributorData) {
      return (
        <Pressable style={styles.dealerUpgradeCard} onPress={handleBecomeDistributor}>
          <View style={styles.dealerUpgradeContent}>
            <Text style={styles.upgradeTitle}>Become a Distributor</Text>
            <Text style={styles.upgradeSub}>Share listings, bring sellers, and earn on approved listings and sold cars.</Text>
          </View>
    ...
    return (
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Distributor Dashboard</Text>
        <View style={styles.verifCenterCard}>
          <Text style={styles.verifyTitle}>Referral Code</Text>
          <Text style={[styles.trustTitle, { marginBottom: 8 }]}>{distributorData?.distributor?.distributorCode || user?.distributorCode || '—'}</Text>
          <Text style={styles.verifySub}>{distributorData?.referralLink || 'Your share link will appear here.'}</Text>
          <View style={{ flexDirection: 'row', gap: 10, marginTop: 14 }}>
            <Pressable style={[styles.promoBtn, { flex: 1 }]} onPress={handleShareDistributorLink}>
              <Text style={styles.promoBtnText}>Share</Text>
            </Pressable>
            <Pressable style={[styles.headerIconButton, { width: 52, height: 44, borderRadius: 12 }]} onPress={() => navigation.navigate('Wallet')}>
              <Ionicons name="wallet-outline" size={22} color={COLORS.black2} />
            </Pressable>
          </View>
        </View>
        ...
        <View style={styles.statsContainer}>
          <View style={styles.statsCard}>
            <StatItem value={distributorData?.stats?.totalReferrals || 0} label="Referrals" color={COLORS.primary} />
            <View style={styles.statDivider} />
            <StatItem value={`₹${(distributorData?.stats?.pendingEarnings || 0).toLocaleString()}`} label="Pending" color={COLORS.coral} />
            <View style={styles.statDivider} />
            <StatItem value={`₹${(distributorData?.stats?.totalEarnings || 0).toLocaleString()}`} label="Total Earned" color={COLORS.success} />
          </View>
        </View>
      </View>
    );
        ...
        {activeTab === 'OVERVIEW' && (
          <>
            ...
            {!!referralRows.length && (
              <View style={styles.section}>
                <View style={styles.sectionHeader}>
                  <Text style={styles.sectionTitle}>Referred Listings</Text>
                  <Text style={styles.viewAllText}>{referralRows.length} total</Text>
                </View>
                {referralRows.slice(0, 5).map((item: any) => (
                  <View key={item.listingId} style={styles.leadCard}>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.leadName}>{item.listingTitle}</Text>
                      <Text style={styles.leadTime}>{item.sellerName || item.sellerPhone || 'Seller'} • {item.status}</Text>
                    </View>
                    <View style={{ alignItems: 'flex-end' }}>
                      <Text style={styles.leadName}>₹{(item.commissionAmount || 0).toLocaleString()}</Text>
                      <Text style={styles.leadTime}>{item.commissionStatus}</Text>
                    </View>
                  </View>
                ))}
              </View>
...
const styles = StyleSheet.create({
  ...
  promoOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 20 },
