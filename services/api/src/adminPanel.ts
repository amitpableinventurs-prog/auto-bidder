...
<aside id="sidebar">
  ...
  <div class="nav-item" data-mod="brands" onclick="nav(this)">
    <span class="nav-icon"><i class="fa-solid fa-tags"></i></span> Brand Management
  </div>
  <div class="nav-item" data-mod="collections" onclick="nav(this)">
    <span class="nav-icon"><i class="fa-solid fa-layer-group"></i></span> Car Collections
  </div>
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
let _collections = [];
_loaders.collections = async function loadCollections() {
  try {
    const data = await apiFetch('/admin/collections/all');
    _collections = data.collections||[];
    renderCollections();
  } catch(e) { showModErr('📦','Collections',e); }
};

window.renderCollections = function() {
  const stats = statGrid([
    {label:'Total Collections',value:_collections.length,icon:'📦',color:'var(--purple)'},
    {label:'Active',value:_collections.filter(c=>c.isActive).length,icon:'✅',color:'var(--green)'},
  ]);
  const cols = [
    {label:'Image',r:r=>r.imageUrl?\`<img src="\${r.imageUrl}" style="width:40px;height:40px;object-fit:cover;border-radius:4px">\`:'—'},
    {label:'Name',r:r=>\`<div style="font-weight:700">\${esc(r.name)}</div>\`},
    {label:'Order',r:r=>esc(r.order)},
    {label:'Status',r:r=>r.isActive?badge('ACTIVE','green'):badge('INACTIVE','red')},
    {label:'Created',r:r=>dtshort(r.createdAt)},
    {label:'',r:r=>\`<button class="action-btn action-btn-view" onclick="event.stopPropagation();showCollectionDrawer(\${JSON.stringify(r).replace(/'/g,'&#39;')})">Edit</button>\`},
  ];
  $('main').innerHTML = \`
    <div class="page-header">
      <div class="page-header-icon" style="background:rgba(168,85,247,.12)">📦</div>
      <div class="page-header-text"><div class="title">Car Collections</div><div class="subtitle">Manage groups of cars shown on home screen</div></div>
      <div class="page-header-actions"><button class="h-btn h-btn-primary" onclick="showAddCollectionModal()">+ Add Collection</button></div>
    </div>
    \${stats}
    \${buildTable(cols, _collections, 'showCollectionDrawer')}
  \`;
};

window.showCollectionDrawer = function(r) {
  if(typeof r==='string') r=JSON.parse(r);
  openDrawer(\`<div class="drawer-title">📦 Edit Collection</div>
    <div style="display:flex;flex-direction:column;gap:12px">
      \${r.imageUrl ? \`<img src="\${r.imageUrl}" style="width:100%;height:120px;object-fit:cover;border-radius:8px;margin-bottom:10px;border:1px solid var(--border)">\` : ''}
      <div>
        <label style="display:block;font-size:11px;font-weight:700;margin-bottom:4px">Name</label>
        <input id="c-name" class="filter-select" style="width:100%" value="\${esc(r.name)}" placeholder="Collection Name">
      </div>
      <div>
        <label style="display:block;font-size:11px;font-weight:700;margin-bottom:4px">Image URL</label>
        <input id="c-img" class="filter-select" style="width:100%" value="\${esc(r.imageUrl)}" placeholder="Image URL">
      </div>
      <div>
        <label style="display:block;font-size:11px;font-weight:700;margin-bottom:4px">Display Order</label>
        <input id="c-order" type="number" class="filter-select" style="width:100%" value="\${r.order}" placeholder="0">
      </div>
      <div style="display:flex;align-items:center;gap:8px">
        <input id="c-active" type="checkbox" \${r.isActive?'checked':''}>
        <label for="c-active" style="font-size:13px">Active</label>
      </div>
      <div style="display:flex;gap:8px;margin-top:10px">
        <button class="h-btn h-btn-primary" onclick="saveCollection('\${r.id}')">Save Changes</button>
        <button class="h-btn h-btn-ghost" style="color:var(--red)" onclick="deleteCollection('\${r.id}')">Delete</button>
      </div>
    </div>
  \`);
};

window.showAddCollectionModal = function() {
  openDrawer(\`<div class="drawer-title">📦 Add New Collection</div>
    <div style="display:flex;flex-direction:column;gap:12px">
      <input id="c-name" class="filter-select" style="width:100%" placeholder="Collection Name (e.g. Budget Cars)">
      <input id="c-img" class="filter-select" style="width:100%" placeholder="Image URL">
      <input id="c-order" type="number" class="filter-select" style="width:100%" value="0" placeholder="Display Order">
      <button class="h-btn h-btn-primary" onclick="saveCollection()">Create Collection</button>
    </div>
  \`);
};
...
window.saveCollection = async function(id) {
  const data = {
    name: $('c-name').value,
    imageUrl: $('c-img').value,
    order: parseInt($('c-order').value)||0,
    isActive: id ? $('c-active').checked : true,
  };
  try {
    const url = id ? '/admin/collections/'+id : '/admin/collections';
    await apiFetch(url, {method: id?'PATCH':'POST', body:JSON.stringify(data)});
    toast(id?'Collection updated':'Collection created');
    closeDrawer(); _loaders.collections();
  } catch(e) { toast(e.message,'err'); }
};
...
window.deleteCollection = async function(id) {
  ...
  try {
    ...
    toast('Collection deleted'); closeDrawer(); _loaders.collections();
  } catch(e) { toast(e.message,'err'); }
};
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
let _brands = [];
_loaders.brands = async function loadBrands() {
  try {
    const data = await apiFetch('/admin/brands/all');
    _brands = data.brands||[];
    renderBrands();
  } catch(e) { showModErr('🏷️','Brands',e); }
};

window.renderBrands = function() {
  const stats = statGrid([
    {label:'Total Brands',value:_brands.length,icon:'🏷️',color:'var(--purple)'},
    {label:'Active',value:_brands.filter(b=>b.isActive).length,icon:'✅',color:'var(--green)'},
  ]);
  const cols = [
    {label:'Logo',r:r=>r.logo?\`<img src="\${r.logo}" style="width:30px;height:30px;object-fit:contain">\`:'—'},
    {label:'Name',r:r=>\`<div style="font-weight:700">\${esc(r.name)}</div>\`},
    {label:'Cars',r:r=>esc(r.count)},
    {label:'Order',r:r=>esc(r.order)},
    {label:'Status',r:r=>r.isActive?badge('ACTIVE','green'):badge('INACTIVE','red')},
    {label:'',r:r=>\`<button class="action-btn action-btn-view" onclick="event.stopPropagation();showBrandDrawer(\${JSON.stringify(r).replace(/'/g,'&#39;')})">Edit</button>\`},
  ];
  $('main').innerHTML = \`
    <div class="page-header">
      <div class="page-header-icon" style="background:rgba(59,130,246,.12)">🏷️</div>
      <div class="page-header-text"><div class="title">Brand Management</div><div class="subtitle">Manage car manufacturers and their logos</div></div>
      <div class="page-header-actions"><button class="h-btn h-btn-primary" onclick="showAddBrandModal()">+ Add Brand</button></div>
    </div>
    \${stats}
    \${buildTable(cols, _brands, 'showBrandDrawer')}
  \`;
};

window.showBrandDrawer = function(r) {
  if(typeof r==='string') r=JSON.parse(r);
  openDrawer(\`<div class="drawer-title">🏷️ Edit Brand</div>
    <div style="display:flex;flex-direction:column;gap:12px">
      \${r.logo ? \`<div style="text-align:center;padding:10px;background:var(--surface2);border-radius:8px;margin-bottom:10px"><img src="\${r.logo}" style="height:60px;object-fit:contain"></div>\` : ''}
      <div>
        <label style="display:block;font-size:11px;font-weight:700;margin-bottom:4px">Brand Name</label>
        <input id="b-name" class="filter-select" style="width:100%" value="\${esc(r.name)}" placeholder="e.g. Maruti Suzuki">
      </div>
      <div>
        <label style="display:block;font-size:11px;font-weight:700;margin-bottom:4px">Logo URL</label>
        <input id="b-logo" class="filter-select" style="width:100%" value="\${esc(r.logo)}" placeholder="URL to png/svg logo">
      </div>
      <div>
        <label style="display:block;font-size:11px;font-weight:700;margin-bottom:4px">Car Count Text</label>
        <input id="b-count" class="filter-select" style="width:100%" value="\${esc(r.count)}" placeholder="e.g. 2k+ Cars">
      </div>
      <div>
        <label style="display:block;font-size:11px;font-weight:700;margin-bottom:4px">Display Order</label>
        <input id="b-order" type="number" class="filter-select" style="width:100%" value="\${r.order}" placeholder="0">
      </div>
      <div style="display:flex;align-items:center;gap:8px">
        <input id="b-active" type="checkbox" \${r.isActive?'checked':''}>
        <label for="b-active" style="font-size:13px">Active</label>
      </div>
      <div style="display:flex;gap:8px;margin-top:10px">
        <button class="h-btn h-btn-primary" onclick="saveBrand('\${r.id}')">Save Changes</button>
        <button class="h-btn h-btn-ghost" style="color:var(--red)" onclick="deleteBrand('\${r.id}')">Delete</button>
      </div>
    </div>
  \`);
};
...
window.showAddBrandModal = function() {
  openDrawer(\`<div class="drawer-title">🏷️ Add New Brand</div>
    <div style="display:flex;flex-direction:column;gap:12px">
      <input id="b-name" class="filter-select" style="width:100%" placeholder="Brand Name">
      <input id="b-logo" class="filter-select" style="width:100%" placeholder="Logo URL">
      <input id="b-count" class="filter-select" style="width:100%" placeholder="e.g. 500+ Cars">
      <input id="b-order" type="number" class="filter-select" style="width:100%" value="0" placeholder="Display Order">
      <button class="h-btn h-btn-primary" onclick="saveBrand()">Create Brand</button>
    </div>
  \`);
};
...
window.saveBrand = async function(id) {
  const data = {
    ...
    logo: $('b-logo').value,
    count: $('b-count').value,
    order: parseInt($('b-order').value)||0,
    isActive: id ? $('b-active').checked : true,
  };
  try {
    const url = id ? '/admin/brands/'+id : '/admin/brands';
    await apiFetch(url, {method: id?'PATCH':'POST', body:JSON.stringify(data)});
    toast(id?'Brand updated':'Brand created');
    closeDrawer(); _loaders.brands();
  } catch(e) { toast(e.message,'err'); }
};
...
window.deleteBrand = async function(id) {
  ...
  try {
    ...
    toast('Brand deleted'); closeDrawer(); _loaders.brands();
  } catch(e) { toast(e.message,'err'); }
};
