...
class DevStore {
  ...
  // User methods
  upsertPhoneUser(phone: string, name = 'Mobile User') {
  ...
  // Collection methods
  listCollections(query: { isActive?: boolean } = {}) {
    return this.collections
      .filter((c) => query.isActive === undefined || c.isActive === query.isActive)
      .sort((a, b) => a.order - b.order || b.createdAt.getTime() - a.createdAt.getTime());
  }

  createCollection(input: Omit<Collection, 'id' | 'createdAt' | 'updatedAt'>) {
    const now = new Date();
  ...
  updateCollection(id: string, patch: Partial<Collection>) {
    ...
    Object.assign(collection, patch, { updatedAt: new Date() });
    return collection;
  }
  ...
  getCollections() {
    const cols = this.listCollections({ isActive: true });
  ...
  getCommissions() {
    return this.payments.filter(p => p.status === 'SUCCEEDED').map(p => ({
      id: id('com'),
      amount: Math.floor(p.amount * 0.05), // 5% commission
      paymentId: p.id,
  ...
  seedRich() {
    ...
    if (true) {
        ...
        const initialCols = [
          { name: 'Budget Cars', imageUrl: 'https://images.unsplash.com/photo-1541899481282-d53bffe3c15d?auto=format&fit=crop&w=400&q=80' },
          { name: 'SUV Cars', imageUrl: 'https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?auto=format&fit=crop&w=400&q=80' },
          { name: 'CNG Cars', imageUrl: 'https://images.unsplash.com/photo-1503376780353-7e6692767b70?auto=format&fit=crop&w=1200&q=80' },
        ];
        initialCols.forEach((c, i) => {
